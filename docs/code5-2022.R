...
mutate = function(xi){
  ### This function adds mutations at each locus with probability u. 
  new = xi + rbinom(Lloci, 1, u)
  1.0*(new>=1)
}

recombine = function(xi, xj){
  ### This function returns a recombined version of genomes xi and xj. 
  joint = c(xi, xj)
...
for (time in seq(1,TF)) {
  ...
  print(c("Generation:", time, " - No. females: ", length(popF.xi), ", No. males: ", length(popM.xi), ", Mean no. of mutations: ",  mean(count(popF.xi))))
